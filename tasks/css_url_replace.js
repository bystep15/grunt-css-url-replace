...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('css_url_replace', 'Grunt task to replace css urls with absolute path', function() {
    ...
    if (options.map && grunt.file.exists(options.map)) {
      map = grunt.file.readJSON(options.map);
    }
    ...
    this.files.forEach(function(f) {
      ...
      var src = f.src.filter(function(filepath) {
        ...
        return new Replace(filepath, options.staticRoot, options.pathRoot, map).run();
      }).join('\n');
